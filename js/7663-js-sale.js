...
const testAcc = "0xF09C50191f10b84b12d954a64142A06e7645BB2E";
var account = null;
var contract1 = null;
var contract2 = null;

var dustBalance = null;
var tokensBalance = null;
var getTokensArray = [];
var totalSupply = "";
var approved = "";
...
const burn = async (e)=> {
	if (typeof window.ethereum !== 'undefined') {
    	const _burnAmount = document.getElementById('tokens_amount').value;
		var arrayShort = [];

		for (var i = 0; i < _burnAmount; i++) {
    		if (getTokensArray[i] != null) {
		    	arrayShort.push(getTokensArray[i]);
        		}
			}

    	if (arrayShort.length > 0) {
			var result = "";
			var success = "";
			document.getElementById("burn").innerHTML = "BUSY"
			try {
			    const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
			    account = accounts[0];
			    ...
			    const gas = Math.round( await contract1.methods.BurnDustAmount(arrayShort).estimateGas({from: account, to: RTS_CONTRACT, }) * 1.1 );
			    result = await contract1.methods.BurnDustAmount(arrayShort).send({from: account, to: RTS_CONTRACT, gas: gas});

				success = document.getElementById("burn").innerHTML = "BURN";
			    } 
		    catch(e)
			    {
			    alert("Error: " + e.message);
...
const getTokens = async (e)=> {
	if (typeof window.ethereum !== 'undefined') {
    	...
    	if (account.length > 0) {
			...
			getTokensArray = await contract2.methods.tokensOfOwner(account).call();

			if (getTokensArray.length > 0) {
				document.getElementById("tokensIds").innerHTML = "Moondust tokens: " + getTokensArray.toString().substr(0,27) + "...";
				document.getElementById("tokensBalance").innerHTML = "Balance: " + getTokensArray.length;
				}
			else
				{
				document.getElementById("tokensIds").innerHTML = "Moondust tokens: No more tokens to burn.";
				document.getElementById("tokensBalance").innerHTML = "Balance: 0";
				}
			}
  		}
  	return false;
